 ...
 * Prevents destructive operations on the Alumimundo database
 */
...
/**
 * Tool 1: List all available tables in the Alumimundo database
...
export const listTablesTool = tool(
  async () => {
    try {
      ...
      return `Available tables in Alumimundo database: ${tables.join(', ')}

Alumimundo Business Data:
- Project: Construction projects (residential, commercial, hospitality, institutional)
- Specification: Product specifications per project with pricing
- Product: Complete product catalog (KOHLER, Schlage, Steelcraft, Kallista)
- QualityCheck: Installation validation with CV analysis
- Document: Generated specifications, quotes, purchase orders, invoices
- InventoryMovement: Stock changes (purchases, sales, adjustments, damages)
- InventoryAllocation: Reserved stock for projects
- Provider: Manufacturers and suppliers (KOHLER, Schlage, etc.)
- ProductImage: Product images stored in GCS
- ScrapeLog: Web scraping history tracking
- Activity: Audit trail and activity log
- User: User management with roles (admin, technical_advisor, architect, installer)
- DesignProject: AI-powered property redesign projects
- DesignArea: Individual rooms/areas within design projects
- AreaImage: Photos for each area with AI analysis
- AreaSpecification: Products selected for each area

Total: ${tables.length} tables with comprehensive construction and design data`;
    } catch (error: any) {
  ...
  },
  {
    ...
    description: "List all available tables in the Alumimundo database. Use this FIRST to see what data is available before generating queries. Returns table names with brief descriptions.",
  }
);
...
export const getSchemaTool = tool(
  ...
  async (input: TablesInput) => {
    try {
      ...
      if (tableList.length === 0) {
        return "Error: No tables specified. Please provide comma-separated table names (e.g., 'Project, Product').";
      }
  ...
  {
    name: "sql_db_schema",
    description: "Get detailed schema information for specific tables including columns, data types, foreign keys, and sample data. Input should be comma-separated table names (e.g., 'Project, Product'). Use this BEFORE writing queries to understand table structure.",
    schema: tablesSchema,
  }
);
