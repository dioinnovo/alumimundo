/**
 * Chart Generator for SQL Analytics - Alumimundo
...
export function determineChartType(
  ...
  const hasDateColumn = columns.some(col => {
    ...
    return (
      ...
      colLower.includes('mes') ||
      colLower.includes('year') ||
      colLower.includes('año') ||
      colLower.includes('week') ||
      colLower.includes('semana') ||
      colLower.includes('day') ||
      colLower.includes('día') ||
      colLower.includes('time') ||
      colLower.includes('tiempo') ||
...
function getMetricType(columnName: string): 'currency' | 'percentage' | 'count' | 'other' {
  ...
  // Currency indicators (English and Spanish)
  const currencyKeywords = [
    'revenue', 'ingresos', 'price', 'precio', 'cost', 'costo', 'amount', 'monto',
    'value', 'valor', 'fee', 'tarifa', 'margin', 'margen', 'profit', 'ganancia',
    'spend', 'gasto', 'budget', 'presupuesto', 'total', 'payment', 'pago'
  ];
  ...
  if (lowerName.includes('rate') || lowerName.includes('tasa') ||
      lowerName.includes('pct') || lowerName.includes('percent') ||
      lowerName.includes('porcentaje') || lowerName.includes('%')) {
    return 'percentage';
  }

  // Count indicators
  const countKeywords = [
    'count', 'conteo', 'total', 'number', 'número', 'cantidad', 'quantity',
    'projects', 'proyectos', 'products', 'productos', 'volume', 'volumen'
  ];
...
export function generateChartData(
  ...
  if (columns.length > 2) {
    // Look for a string column (like product name, project name, etc.)
  ...
  const labels = rows.map(row => {
    ...
    if (value instanceof Date) {
      return value.toLocaleDateString('es-CR', { month: 'short', day: 'numeric' });
    }
  ...
  const title = displayedMetrics.length === 1
    ? `${displayedMetrics[0]} por ${labelColumnName}`
    : `${displayedMetrics.join(', ')} por ${labelColumnName}`;
...
export function formatColumnName(columnName: string): string {
  // Handle common abbreviations (English and Spanish)
